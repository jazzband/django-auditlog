...
from auditlog.models import LogEntry, _manager_from_settings
...
def log_update(sender, instance, **kwargs):
    ...
    if not instance._state.adding and instance.pk is not None:
        ...
        old = _manager_from_settings(sender).filter(pk=instance.pk).first()
        _create_log_entry(
...
def make_log_m2m_changes(field_name):
    ...
    def log_m2m_changes(signal, action, **kwargs):
        ...
        model_manager = _manager_from_settings(kwargs["model"])
